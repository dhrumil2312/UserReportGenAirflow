...
hive -e "insert into user_total select current_timestamp() , cnt , if(total_users is null , cnt , cnt-total_users) from (select count(1) cnt from users ) a left join (select * from user_total b left join (select max(time_ran) max_time_ran from user_total) c on (b.time_ran = c.max_time_ran) )b on 1=1;"
